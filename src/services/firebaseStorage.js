...
import {addDoc, collection, deleteDoc, doc, getDoc, getDocs, getFirestore, setDoc, updateDoc } from '@react-native-firebase/firestore';
...
export const addTask = async (email, taskData, isCompleted = false) => {
    try {
        ...
        console.log(`Task added to ${taskType} successfully!`);
        return true;
    } catch (error) {
...
export const getTasks = async (email, isCompleted = false) => {
    try {
        const taskType = isCompleted ? 'CompletedTasks' : 'OnGoingTasks';
        const tasksCollection = collection(db, 'TaskManager','shubhamp4578@gmail.com',taskType);
        const querySnapshot = await getDoc(tasksCollection);
        const tasks = querySnapshot.docs.map((value) => ({id:value.id, ...value.data()}));
        console.log(`Fetched ${taskType}: `,tasks);
        return tasks;
    } catch (error) {
...
export const updateTask = async (email, taskId, updatedData, isCompleted = false) => {
    try {
        const taskType = isCompleted ? 'CompletedTasks' : 'OnGoingTasks';
...
export const moveTaskToCompleted = async (email, taskId) => {
    try {
        ...
        if(taskSnap.exists()){
            ...
            await addDoc(collection(db, 'TaskManager', email, 'CompletedTasks'),taskData);
            await deleteDoc(taskRef);
...
export const deleteUser = async (email) => {
    try {
        const userRef = doc(db, 'TaskManager',email);
        const ongoingTaskCollection = collection(db,'TaskManager',email,'OnGoingTasks');
        ...
        const completedTaskCollection = collection(db,'TaskManager',email,'CompletedTasks');
